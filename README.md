...
├── event_watch.py          급변 종목 재분석 트리거 (30일 주기 무시)
...
- 매시각 정각(:00) 자동 실행 — failed 큐 복귀 → **급변 종목 재투입** → 핫 종목 5개 enqueue → 워커가 순차 분석

**급변 종목 재분석**

한 번 분석한 종목은 30일간 재분석 대상에서 빠지는데(같은 종목 반복 방지), 그 사이 **오늘 ±15% 움직이거나 52주 신고가를 경신하면 주기를 무시하고 그날 바로 다시 분석한다.** "왜 오늘 튀었는지, 무슨 뉴스가 있었는지"가 분석의 1순위 과제가 되고, 직전 회차 결론과 무엇이 달라졌는지도 함께 짚는다. 이 종목들은 큐에서 정기 핫픽보다 먼저 처리되고, 등급과 무관하게 텔레그램으로 온다.

하루 건수 상한이나 쿨다운은 없다 — **이틀 연속 급등하면 이틀 다 분석한다.** 단 같은 날 같은 종류의 이벤트로 두 번 부르지는 않으며, 오늘 이미 분석한 종목은 오늘 다시 부르지 않는다. 대시보드 상단 `⚡ 급변 종목 감지` 버튼으로 수동 실행도 가능하다. 감지 기준은 `.env`의 `EVENT_SURGE_PCT`(기본 15) 등으로 조정한다 — 이 값을 낮추면 감지량이 급격히 늘어 큐가 밀리므로 주의(±10%면 하루 30종목 이상).
...
python event_watch.py           # 오늘 급변한 '이미 분석한 종목' 감지 (큐 미투입)
...
> 알림 정책은 **소스별로 다름**: 자동 스크리닝(`auto_hourly`)은 **STRONG** 이거나 **향후 1~2년 실적변화 이벤트**(공장 증설·신공장 준공·독점 공급 계약·신규 양산·해외 생산·대규모 수주·고객사 증설 등)가 잡힌 종목만 푸시(스팸 방지). 단 **SKIP 등급은 이벤트가 있어도 푸시하지 않는다**(이벤트 푸시 대상은 실질적으로 INTEREST 이상). 사용자가 직접 지정한 종목(`manual`/`telegram`)은 등급·이벤트와 무관하게 항상 푸시. **급변 재분석(`auto_event`)도 등급 무관 항상 푸시** — 애초에 "오늘 왜 튀었는지"를 알려고 부른 종목이라 SKIP이면 '올랐지만 살 이유는 없다'가 곧 알고 싶던 답이다.

> 푸시는 종목당 **메시지 2건**으로 나눠 온다(밖에서 대시보드 링크를 못 여는 상황 대비). ① **요약** — 종목·등급·평균★·실적변화 이벤트 한 줄·전문 링크 + 분석일/시장/종가 + 한 줄 요약 + 영역별 매트릭스(점수와 함께 **영역별 요지**·평균까지). ② **상세** — ①에 담기지 않은 **나머지 섹션 전부**(당일 급등 사유·핵심 포인트 긍정/부정·단기 트리거·중장기 실적변화 이벤트 정량평가·모니터링 우선순위·등급 근거·이전 회차 대비 변동)를 **요약 없이 그대로**. 두 건을 합치면 보고서 내용이 빠짐없이 전달되므로, ①만 읽고 넘길지는 취향대로(`notifier.build_single_messages`). 전문은 여전히 대시보드에.
>
> 실적변화 이벤트에 붙는 두 지표: **레버리지 A~D** = 이벤트가 1~2년 실적을 바꾸는 **크기**(A: 연매출 대비 +30%↑·흑자전환급 / B: +10~30% / C: +3~10% / D: 3% 미만·규모 미공개). **신뢰도 ★1~5** = 그 실적 변화가 실제로 일어날 **확실성**(★5: confirmed + 수주확보·실적반영 + 금액 명확 / ★3: expected + 투자진행·증설완료 + 대략 규모 / ★1: rumor·규모 미공개·계획 단계뿐).
...
```
[매시각 정각(:00) 트리거]  ── failed 큐 복귀 → 급변 종목 재투입 → 핫 종목 enqueue → 워커 순차 처리
        │                              │
        │                              ▼
        │                       event_watch.py ◄── 최근 30일 분석 종목의 일봉만 재확인
        │                              (오늘 ±15% 또는 52주 신고가 → 30일 dedup 무시하고
        │                               큐 맨 앞으로. 오늘 이미 분석한 종목은 제외)
        ▼
