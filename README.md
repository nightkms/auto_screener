...
> 알림 정책은 **소스별로 다름**: 자동 스크리닝(`auto_hourly`)은 **STRONG** 이거나 **향후 1~2년 실적변화 이벤트**(공장 증설·신공장 준공·독점 공급 계약·신규 양산·해외 생산·대규모 수주·고객사 증설 등)가 잡힌 종목만 푸시(스팸 방지). 등급이 SKIP이어도 이런 이벤트가 있으면 푸시된다. 사용자가 직접 지정한 종목(`manual`/`telegram`)은 등급·이벤트와 무관하게 항상 푸시.
