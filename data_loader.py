...
DOC_FETCH_TITLE_PATTERNS = (
    ...
    # 고신호 거래소/실적 공시 — 제목만으론 증감 방향·규모가 안 드러남
    "배당결정", "손익구조", "기업가치제고",       # 배당 증감·실적 급변(매출/손익 변경)·밸류업
    "단일판매", "공급계약", "수주",              # 대형 수주/공급계약
)
...
def should_fetch_document(d: dict) -> bool:
    ...
    - 위 유형 판정이 누락돼도 제목 패턴으로 한 번 더 거른다.
      배당결정(증감 방향·금액)·손익구조 변경(실적 급변)·기업가치제고(밸류업)·
      대형 공급계약/수주는 거래소 공시라 유형으론 안 잡혀도 제목 패턴으로 포함한다.
